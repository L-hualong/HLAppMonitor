...
+ (NSString *)td_backtraceOfAllThread {
    ...
    for(int i = 0; i < thread_count; i++) {
        [resultString appendString:_td_backtraceOfThread(threads[i])];
    }
...
NSString *_td_backtraceOfThread(thread_t thread) {
    ...
    for (int i = 0; i < backtraceLength; ++i) {
        [resultString appendFormat:@"%@", td_logBacktraceEntry(i, backtraceBuffer[i], &symbolicated[i])];
    }
...
NSString* td_logBacktraceEntry(const int entryNum,
...
uintptr_t td_mach_framePointer(mcontext_t const machineContext){
    return machineContext->__ss.TD_FRAME_POINTER;
}

uintptr_t td_mach_stackPointer(mcontext_t const machineContext){
    return machineContext->__ss.TD_STACK_POINTER;
}

uintptr_t td_mach_instructionAddress(mcontext_t const machineContext){
    return machineContext->__ss.TD_INSTRUCTION_ADDRESS;
}

uintptr_t td_mach_linkRegister(mcontext_t const machineContext){
...
kern_return_t td_mach_copyMem(const void *const src, void *const dst, const size_t numBytes){
    vm_size_t bytesCopied = 0;
    return vm_read_overwrite(mach_task_self(), (vm_address_t)src, (vm_size_t)numBytes, (vm_address_t)dst, &bytesCopied);
}

#pragma -mark Symbolicate
void td_symbolicate(const uintptr_t* const backtraceBuffer,
    ...
    if(!skippedEntries && i < numEntries) {
        td_dladdr(backtraceBuffer[i], &symbolsBuffer[i]);
        i++;
    }
    
    for(; i < numEntries; i++) {
        td_dladdr(CALL_INSTRUCTION_FROM_RETURN_ADDRESS(backtraceBuffer[i]), &symbolsBuffer[i]);
    }
}
...
bool td_dladdr(const uintptr_t address, Dl_info* const info) {
    ...
    const uintptr_t segmentBase = td_segmentBaseOfImageIndex(idx) + imageVMAddrSlide;
    if(segmentBase == 0) {
        return false;
    }
